...
export default {
  ...
  actions: {
    ...
    openedLoad ({ state, commit, dispatch }) {
      return new Promise(async resolve => {
        ...
        state.opened = value.map(opened => {
          // 忽略首页
          if (opened.fullPath === '/statistics/survey/index') {
            valid.push(1)
            return opened
          }
    ...
    closeAll ({ state, commit, dispatch }) {
      return new Promise(async resolve => {
        ...
        if (router.app.$route.name !== '/statistics/survey/index') {
          router.push({
            name: 'statistics-survey-index'
          })
        }
        // end
        resolve()
  ...
  mutations: {
    ...
    currentSet (state, fullPath) {
      ...
      if (!fullPath.includes('login') && !fullPath.includes('500')) {
        util.cookies.set('lastLeaveRoute', fullPath)
      }
    },
    /**
     * @class pool
