                ...
                <a-row :gutter="12">
                    <a-col :md="4">
                        <a-form-item label="渠道">
                            <a-select showSearch optionFilterProp="children" :filterOption="filterOption" v-decorator="['channel_id']" placeholder="请选择" name="c">
...
export default {
    mixins: [mixin],
    name: 'statistics-survey-stageAmountRatioSingleStroke',
    components: {
        MddTable,
    },
    data() {
        return {
            ...
            loadData: parameter => {
                return this.$api.OVERVIEW_STAGE_AMOUNT_RATIO_SINGLE_STROKE_ALL(this.queryParam)
                    .then(res => {
                        let data = {
                            data: []
                        }
        ...
        handleExport() {
            this.form.validateFields((err, values) => {
                if (!err) {
                    ...
                    this.$api.OVERVIEW_STAGE_AMOUNT_RATIO_SINGLE_STROKE_EXPORT({
                        ...values,
                        ...params
                    }).then(res => {
